...
def get_progress_function(n_taggers):
    ...
    prog_string = '\r {0:3d}  [{1}{2}]   {3:5.1f}% {4:8.1f}s'
    accu_string = [' {{{0}{1}}}%'.format(j+5,':9.2f') for j in range(n_taggers)]
    prog_string += redlist(accu_string)

    def progress(epoch,batchnum,batchcnt,corrects,total,stime,alen = 40):
        percent = 100.0*(batchnum+1)/batchcnt
        elapsed = time.time()-stime
        denom = 0 if total == 0 else 1.0/total
        accuracies = mullist(100.0,mullist(denom,corrects))
        n = int(percent*alen//100)
        fmat = [epoch+1,'|'*n,'-'*(alen-n),percent,elapsed]+accuracies
        print(prog_string.format(*fmat),end = '\r' if percent < 100 else '\n')

    prog_header = 'Epoch '+' '*40+'   Complete   Elapsed'
    accu_header = ['   Accu(#{0})'.format(j) for j in range(n_taggers)]
